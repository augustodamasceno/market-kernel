...
#ifndef _B3_SETTLEMENTPRICE_28_CXX_H_
#define _B3_SETTLEMENTPRICE_28_CXX_H_
...
public:
    ...
    {
        if (m_actingVersion < 17)
        {
            return static_cast<std::uint16_t>(65535);
        }

        std::uint16_t val;
        std::memcpy(&val, m_buffer + m_offset + 35, sizeof(std::uint16_t));
        return SBE_LITTLE_ENDIAN_ENCODE_16(val);
    }

    SettlementPrice_28 &settlDate(const std::uint16_t value) SBE_NOEXCEPT
    {
        std::uint16_t val = SBE_LITTLE_ENDIAN_ENCODE_16(value);
        std::memcpy(m_buffer + m_offset + 35, &val, sizeof(std::uint16_t));
        return *this;
    }
